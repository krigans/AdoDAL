...
Public Class Utilities
    ...
    Public Shared Sub LogSystemError(ex As Exception)
        ...
        WriteToFile(FilePath & "ErrorLogFile", "Error Date " & DateTime.Now.ToString() & vbCrLf, ".txt")
        WriteToFile(FilePath & "ErrorLogFile", ex.ToString() & vbCrLf & vbCrLf & ex.StackTrace() & vbCrLf, ".txt")
        WriteToFile(FilePath & "ErrorLogFile", "--------------------------------------------------" & vbCrLf, ".txt")
    End Sub
    ...
    Public Shared Sub LogSystemError(ex As Exception, sErrorMessage As String)
        ...
        WriteToFile(FilePath & "ErrorLogFile", "Error Date" & DateTime.Now.ToString() & vbCrLf, ".txt")
        WriteToFile(FilePath & "ErrorLogFile", sErrorMessage & vbCrLf & ex.ToString() & vbCrLf & ex.StackTrace(), ".txt")
        WriteToFile(FilePath & "ErrorLogFile", "--------------------------------------------------" & vbCrLf, ".txt")
    End Sub

    Public Shared Sub LogSystemError(ex As Exception, sErrorMessage As String, sFilePath As String)
        'Console.WriteLine(ex.Message & vbCrLf & ex.StackTrace)
        'FilePath = System.AppDomain.CurrentDomain.BaseDirectory & "\"
        WriteToFile(sFilePath & "ErrorLogFile", "Error Date" & DateTime.Now.ToString() & vbCrLf, ".txt")
        WriteToFile(sFilePath & "ErrorLogFile", ex.ToString() & vbCrLf & vbCrLf & ex.StackTrace() & vbCrLf, ".txt")
        WriteToFile(sFilePath & "ErrorLogFile", "--------------------------------------------------" & vbCrLf, ".txt")
    End Sub

    'Public Shared Sub WriteTextFile(sFileName As String, sText As String)
    '    Try
    '        'IO.File.WriteAllText(sFileName & ".txt", sText)
    '        IO.File.AppendAllText(sFileName & ".txt", sText)
    '    Catch ex As Exception
    '        Throw ex
    '    End Try
    'End Sub

    Public Shared Sub WriteToFile(sFileName As String, sText As String, ByVal sExtension As String)
        Try
            'IO.File.AppendAllText(sFileName & ".txt", sText)
            IO.File.AppendAllText(sFileName & sExtension, sText)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Public Shared Sub WriteToFile(sFileName As String, sText As String, ByVal sExtension As String, ByVal sFilePath As String)
        Try
            'IO.File.AppendAllText(sFileName & ".txt", sText)
            If sFilePath <> "" Then
                IO.File.AppendAllText(sFilePath & sFileName & "." & sExtension, "Log Date" & DateTime.Now.ToString() & vbCrLf & vbCrLf)
                IO.File.AppendAllText(sFilePath & sFileName & "." & sExtension, sText & vbCrLf)
                IO.File.AppendAllText(sFilePath & sFileName & "." & sExtension, "--------------------------------------------------" & vbCrLf)
            Else
                IO.File.AppendAllText(FilePath & sFileName & "." & sExtension, "Log Date" & DateTime.Now.ToString() & vbCrLf & vbCrLf)
                IO.File.AppendAllText(FilePath & sFileName & "." & sExtension, sText & vbCrLf)
                IO.File.AppendAllText(FilePath & sFileName & "." & sExtension, "--------------------------------------------------" & vbCrLf)
            End If
        Catch ex As Exception
            Throw ex
        End Try
